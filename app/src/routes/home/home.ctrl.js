...
const process = {
  ...
  register: async (req, res) => {
    ...
    const url = {
      method: "POST",
      path: "/register",
      status: response.err ? 400 : 201,   // 가입은 데이터 생성 => 201
    };
...
const process = {
  login: async (req, res) => {
    ...
    if (response.err)
      ...
      logger.info (
      `POST /login 200 Response: "누가출석했죠?: ${req.body.id}, success: ${response.success}, msg: ${response.msg}"`
    );
    return res.json(response);
  },
  register: async (req, res) => {
